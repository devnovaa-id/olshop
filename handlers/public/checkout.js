  ...
  try {
    ...
    for (const item of items) {
      ...
      if (!product || product.stock < item.quantity) {
        return { statusCode: 400, body: { message: `Stok tidak cukup untuk produk ID ${item.product_id}` } };
      }
    }
    ...
    return { statusCode: 201, body: { orderNumber, message: 'Pesanan berhasil dibuat' } };
  } catch (err) {
